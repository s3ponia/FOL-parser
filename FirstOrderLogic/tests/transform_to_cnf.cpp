...
TEST_CASE("test basic patterns", "[transform][fol]") {
  ...
  REQUIRE(check::Pred()(transform::Normalize(std::move(fol))));

  fol = Parse(lexer::Tokenize("~(~pP1(vx))"));
  REQUIRE(check::Pred()(transform::Normalize(std::move(fol))));

  fol = Parse(lexer::Tokenize("~(pP1(vx) or pP2(vx))"));
  auto cnf_fol = transform::Normalize(std::move(fol));
  ...
  REQUIRE(check::Disj(check::Not(check::Pred()), check::Not(check::Pred()))(
      transform::Normalize(std::move(fol))));

  fol = Parse(lexer::Tokenize("~(@ vx . pP1(vx))"));
  cnf_fol = transform::Normalize(std::move(fol));
...
TEST_CASE("test complex patterns", "[transform][fol]") {
  ...
  std::cout << fol << " => ";
  fol = transform::Normalize(std::move(fol));
  ...
  std::cout << fol << " => ";
  std::cout << transform::Normalize(std::move(fol)) << std::endl;
